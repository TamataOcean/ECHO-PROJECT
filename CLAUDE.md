...
Le container `echo-project` est **unique** — supervisord y gère mosquitto, gstd, Node-Red et le serveur Python. Il n'y a plus de containers séparés echo-core / echo-ui / portainer.
...
| `NAS_HOST` | `192.168.0.17` | IP/hostname du NAS — lu au démarrage de Node-Red via `env_file` |

Le fichier `docker/.env` est monté en `/app/config.env` dans le container. `NAS_HOST` est lu comme variable d'environnement par Node-Red au démarrage (nœud "Variable Globale", `tot: env`). La sauvegarde depuis l'UI écrit dans `/app/config.env` (= `.env`) et relit le global.
...
**Rebuild requis** si le `Dockerfile` change (ex : ajout de paquets) :
```bash
...
- **⚠️ Édition de `flows.json`** : Node-Red écrase `docker/node-red/data/flows.json` à chaque arrêt propre (sauvegarde de son état mémoire). Pour que des modifications manuelles du fichier survivent, **toujours stopper le container avant d'éditer** (`docker stop echo-project`), puis le redémarrer (`docker start echo-project`). Ne jamais utiliser `docker restart` (stop + start en un seul appel est OK).
- **`iputils-ping`** est installé dans le Dockerfile — requis pour le nœud exec ping (`/usr/bin/ping`) dans le flow "Statut Stockage" NAS.
- Le ping NAS dans Node-Red utilise un nœud exec shell (`/usr/bin/ping -c1 -W1 <host>`) plutôt que le nœud `node-red-node-ping` (ce dernier ne recevait pas correctement les messages entrants). Le résultat est parsé via `"1 received"` dans stdout.
